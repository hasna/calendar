...
export const CALENDAR_STORAGE_TABLES = STORAGE_TABLES;
...
export const CALENDAR_STORAGE_ENV = "HASNA_CALENDAR_DATABASE_URL";
export const CALENDAR_STORAGE_FALLBACK_ENV = "CALENDAR_DATABASE_URL";
export const CALENDAR_STORAGE_MODE_ENV = "HASNA_CALENDAR_STORAGE_MODE";
export const CALENDAR_STORAGE_MODE_FALLBACK_ENV = "CALENDAR_STORAGE_MODE";
export const STORAGE_DATABASE_ENV = [CALENDAR_STORAGE_ENV, CALENDAR_STORAGE_FALLBACK_ENV] as const;
export const STORAGE_MODE_ENV = [CALENDAR_STORAGE_MODE_ENV, CALENDAR_STORAGE_MODE_FALLBACK_ENV] as const;
...
export function getStorageMode(): StorageMode {
  const mode = normalizeStorageMode(
    readEnv(CALENDAR_STORAGE_MODE_ENV)
      ?? readEnv(CALENDAR_STORAGE_MODE_FALLBACK_ENV),
  );
  if (mode) return mode;
...
export async function getStoragePg(): Promise<PgAdapterAsync> {
  ...
  if (!url) {
    throw new Error("Missing HASNA_CALENDAR_DATABASE_URL or CALENDAR_DATABASE_URL");
  }
...
export function getSyncMetaAll(): SyncMeta[] {
  ...
  return db.query("SELECT table_name, last_synced_at, direction FROM _calendar_sync_meta ORDER BY table_name, direction").all() as SyncMeta[];
}
...
export function resolveTables(tables?: string[]): StorageTable[] {
  ...
  if (invalid.length > 0) throw new Error(`Unknown calendar sync table(s): ${invalid.join(", ")}`);
...
function recordSyncMeta(db: Database, direction: "push" | "pull", results: SyncResult[]): void {
  ...
  const statement = db.query(`
    INSERT INTO _calendar_sync_meta (table_name, last_synced_at, direction)
    VALUES (?, ?, ?)
...
function ensureSyncMetaTable(db: Database): void {
  db.exec(`
    CREATE TABLE IF NOT EXISTS _calendar_sync_meta (
