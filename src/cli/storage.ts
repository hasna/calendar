...
function shouldPrintJson(opts: { json?: boolean }, program: Command): boolean {
...
export function registerStorageCommands(program: Command): void {
  ...
  storageCmd
    ...
    .action((opts: { json?: boolean }) => {
      try {
        ...
        if (shouldPrintJson(opts, program)) {
          printJson(info);
          return;
        }
  ...
  storageCmd
    .command("push")
    .description("Push local calendar data to storage PostgreSQL")
  ...
  storageCmd
    .command("pull")
    .description("Pull calendar data from storage PostgreSQL to local SQLite")
    .option("--tables <tables>", "Comma-separated table names (default: all)")
    .option("--json", "Output as JSON")
    .action(async (opts: { tables?: string; json?: boolean }) => {
      try {
